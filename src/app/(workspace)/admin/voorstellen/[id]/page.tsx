...
import { ProposalEditor as AanvraagProposalEditor } from "@/app/(workspace)/admin/voorstellen/proposal-editor";
...
import { loadProposal as loadAanvraagProposal } from "@/lib/aanvragen";
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const [detail, draft] = await Promise.all([loadProposal(id), loadAanvraagProposal(id)]);
  return {
    title: detail?.proposal.title || draft?.title || "Voorstel",
...
export default async function AdminProposalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const proposal = await loadAanvraagProposal(id);
