...
export function toneForStatus(status: string): "olive" | "copper" | "ink" {
    if (["nieuw", "NIEUW", "lead", "voorbereiding", "missing", "rejected", "open"].includes(status)) return "copper";
    if (["live", "klaar", "OMGEZET", "active", "opgeleverd", "ready", "approved"].includes(status)) return "ink";
  return "olive";
}
